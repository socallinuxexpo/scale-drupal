 ...
 *   @see https://github.com/lazymozek/gulp-with-tailwindcss/tree/main
 **/
...
function livePreview(done) {
  browserSync.init({
    proxy: {
      target: "https://scale10.lndo.site"
    },
    open: false,
...
//Development Tasks
function devHTML() {
  return src(`${options.paths.src.theme}/**/*.twig`)
...
function devThirdParty() {
  return src(`${options.paths.src.thirdParty}/**/*`)
    .pipe(dest(options.paths.dist.thirdParty)
  );
}

function devFlowbite() {
  return src(`${options.paths.flowbitejs}/dist/*`)
    .pipe(dest(`${options.paths.dist.thirdParty}/flowbite`));
}
...
function prodHTML() {
  return src(`${options.paths.src.theme}/**/*.{html,php}`)
    .pipe(includePartials())
    .pipe(dest(options.paths.dist.base));
}
...
exports.default = series(
  ...
  parallel(devStyles, devScripts, devImages, devFonts, devThirdParty, devFlowbite, devHTML), //Run All tasks in parallel
